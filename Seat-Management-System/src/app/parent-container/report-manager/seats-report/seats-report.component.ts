...
@Component({
  selector: 'app-seats-report',
  templateUrl: './seats-report.component.html',
  styleUrls: ['./seats-report.component.scss'],
})
export class SeatsReportComponent implements OnInit {
  ...
  onSubmit() {
    ...
    // };
    // console.log(request);
    // this.seatService.getApiSeatsReport(request).subscribe((result) => {
    ...
    this.seatService.getApiSeatsReport(isUnallocatedReport,cityCode,buildingCode,facilityName,floorNumber).subscribe((result) => {
      this.results = result as unknown as any[];
    });
  }
}
